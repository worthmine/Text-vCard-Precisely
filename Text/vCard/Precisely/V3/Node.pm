...
has value => ( is => 'rw', required => 1, isa => 'VALUE|ArrayRef' );
...
subtype 'Type'
    ...
    => where {
        ...
        m/^(:?contact|acquaintance|friend|met|co-worker|colleague|co-resident|neighbor|child|parent|sibling|spouse|kin|muse|crush|date|sweetheart|me|agent|emergency)$/s or    # 本当にこれでいのか怪しい
...
sub as_string {
    ...
    return join(';', @lines ) . ':' . Text::vCard::Precisely::V3::_escape($self->value) unless ref $self->value eq 'Array';
    return join(';', @lines ) . ':' . join ';', map{ Text::vCard::Precisely::V3::_escape($_) } @{ $self->value };
}
