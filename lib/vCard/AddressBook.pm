...
sub _create_vcards {
    ...
    foreach my $vcard_data (@$vcards_data) {
        ...
        my $text_vcard = Text::vCard::Precisely::V3->new(
        {       %{ $vcard_data->{properties} },
                encoding_out => $self->encoding_out,
            }
        );
...
sub _copy_photo {
    my ( $self, $text_vcard, $vcard ) = @_;
    $vcard->photo( $text_vcard->photo );
}

sub _copy_name {
    my ( $self, $text_vcard, $vcard ) = @_;

    my ($node) = $text_vcard->n;

    $vcard->family_names(       [ $node->[0] || () ] );
    $vcard->given_names(        [ $node->[1] || () ] );
    $vcard->other_names(        [ $node->[2] || () ] );
    $vcard->honorific_prefixes( [ $node->[3] || () ] );
    $vcard->honorific_suffixes( [ $node->[4] || () ] );
}
